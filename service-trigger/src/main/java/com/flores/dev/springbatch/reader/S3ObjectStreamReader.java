 ...
 */
@Slf4j
public class S3ObjectStreamReader implements ItemStreamReader<Employee> {

	private String resource;
	
	private S3Client client;

	private ItemStreamReader<Employee> delegate;
	 ...
	 */
	private ItemStreamReader<Employee> getDelegate() throws URISyntaxException {
		if(Optional.ofNullable(delegate).isEmpty()) {			
			...
			//setup reader with customizable mapper
			ObjectMapper mapper = new ObjectMapper();
			JacksonJsonObjectReader<Employee> reader = new JacksonJsonObjectReader<>(mapper, 
					Employee.class);

			delegate = new JsonItemReaderBuilder<Employee>()
					.resource(new InputStreamResource(response))
					.name("StreamedEmployeeJsonReader")
